...
#include <Guid/AppleLoginPointerImageFvFile.h>

#include "Macros.h"
#include "EfiLoginPointerBmp.h"
  ...
  )
{
  ...
  Status = FirmwareVolume2->GetVolumeAttributes (FirmwareVolume2,
                                                 Attributes);
  if (!EFI_ERROR (Status)) {
  ...
  )
{
  ...
  Status = FirmwareVolume2->SetVolumeAttributes (FirmwareVolume2,
                                                 &Fv2Attributes);
  ...
  )
{
  ...
  Status = FirmwareVolume2->ReadFile (FirmwareVolume2,
                                      NameGuid,
                                      Buffer,
                                      BufferSize,
                                      FoundType,
  ...
  DEBUG ((DEBUG_VERBOSE, "%a: %G %d - %r\n", __FUNCTION_NAME__, NameGuid, BufferSize, Status));
  ...
  )
{
  ...
  Status = FirmwareVolume2->ReadSection (FirmwareVolume2,
                                         NameGuid,
                                         SectionType,
                                         SectionInstance,
                                         Buffer,
  ...
  if (EFI_ERROR(Status)) {
    ...
    if (CompareGuid (NameGuid, &gAppleLoginPointerImageFvGuid)) {

      // Inject Internal Pointer Bitmap '1A10742F-FA80-4B79-9DA6-357058CC397B' FvFile

      *Buffer = &mEfiLoginPointerBmp;
      *BufferSize = EfiLoginPointerBmpLength;

      DEBUG ((DEBUG_VERBOSE, "Injecting %G\n", &gAppleLoginPointerImageFvGuid));
  ...
  )
{
  ...
  Status = FirmwareVolume2->WriteFile (FirmwareVolume2,
                                       NumberOfFiles,
                                       WritePolicy,
  ...
  )
{
  ...
  Status = FirmwareVolume2->GetNextFile (FirmwareVolume2,
                                         Key,
                                         FileType,
                                         NameGuid,
                                         Attributes,
  ...
  *Attributes = *Attributes & EFI_FV_FILE_ATTRIB_ALIGNMENT;
  ...
  DEBUG ((DEBUG_VERBOSE, "%a: %G - %r\n", __FUNCTION_NAME__, NameGuid, Status));
  ...
  )
{
  ...
  while (TRUE) {
    ...
    //
    // Override FvReadSection Function If the Firmware Volume Protocol is already installed
    //
    ...
    if (!EFI_ERROR (Status)) {
      ...
      // Override Existing Protocol Functions

      if (FirmwareVolume->ReadSection != FvReadSection) {

        DEBUG ((DEBUG_INFO, "%a: Override Existing FvReadSection %x -> %x\n",
                            __FUNCTION_NAME__,
                            FirmwareVolume->ReadSection,
                            FvReadSection));

        FirmwareVolume->ReadSection = FvReadSection;
